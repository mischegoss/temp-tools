...
from app.prompts.gemini_prompts import (
    ...
    NO_CONTEXT_RESPONSE,
    SUPPORT_RESPONSE,
    ACCOUNT_ISSUE_RESPONSE,
    SYSTEM_ISSUE_RESPONSE,
    BILLING_ISSUE_RESPONSE,
    BILLING_FOLLOWUP_RESPONSE,
    URGENT_ISSUE_RESPONSE,
    BUG_REPORT_RESPONSE
)
...
class GeminiService:
    ...
    def detect_question_type(self, message: str) -> str:
        """Enhanced question type detection with comprehensive support routing"""
        if not message:
            return "general"
            
        message_lower = message.lower()
        
        # Define patterns directly in method to avoid import issues
        question_patterns = {
            ...
            "account_issue": [
                ...
                "change password", "update password", "can't access", "cannot access", "account locked", 
                "account disabled", "account suspended", "permission denied", "access denied", "not authorized",
                "account expired", "account blocked", "two factor", "2fa", "authentication", "mfa not working", 
                "authenticator", "change email", "update email", "change profile", "update profile",
            ...
            "system_issue": [
                ...
                "my workflows", "workflow stopped", "workflow problem", "database error", 
                "connection failed", "network error", "api error", "internal server error", "500 error", "503 error"
            ],
            ...
            # Follow-up billing issues for users who don't have CSM info - MORE SPECIFIC
            "billing_followup": [
                "don't have that information", "don't have their email", "don't know my csm", 
                "no customer success", "don't have csm info", "missing csm contact",
                "don't have the email", "no csm email", "customer success manager email",
                "don't know customer success", "missing customer success contact"
            ],
        ...
        # Check for billing follow-up first (more specific)
        followup_patterns = question_patterns.get("billing_followup", [])
        if any(pattern in message_lower for pattern in followup_patterns):
            return "billing_followup"
        
        # Critical issues 
        for question_type in ["urgent_issue", "system_issue", "account_issue", "billing_issue", "bug_report", "support_request"]:
            patterns = question_patterns.get(question_type, [])
            if any(pattern in message_lower for pattern in patterns):
                return question_type
        
        # Then documentation-searchable questions
        for question_type in ["how_to", "definition", "configuration", "example", "troubleshooting"]:
            patterns = question_patterns.get(question_type, [])
            if any(pattern in message_lower for pattern in patterns):
                return question_type
    ...
    def needs_disambiguation(self, message: str, search_results: List[Any]) -> Tuple[bool, str, List[str]]:
        """
        Detect if query needs disambiguation based on existing data
        Returns: (needs_disambiguation, reason, clarification_options)
        """
        message_lower = message.lower().strip()
        
        # Scenario 1: Very short, vague queries (more selective)
        if len(message.split()) <= 2:  # More restrictive - only 1-2 words
            vague_terms = ['workflow', 'activity', 'action', 'setup', 'configure', 'help', 'issue', 'problem']
            if any(term in message_lower for term in vague_terms):
                return True, "vague_query", self._get_vague_clarifications(message_lower)
        
        # Scenario 2: Multiple strong matches with different content types
        if len(search_results) >= 3:
            content_types = set()
            for result in search_results[:5]:
                if hasattr(result, 'content_type'):
                    content_types.add(getattr(result, 'content_type', 'unknown'))
            
            if len(content_types) >= 3:
                return True, "multiple_contexts", list(content_types)
        
        # Scenario 3: Low similarity scores (user might be confused) - lowered threshold
        if search_results and len(search_results) > 0:
            top_similarity = getattr(search_results[0], 'similarity_score', 0)
            if top_similarity < 0.25:  # Lowered from 0.4 to 0.25
                return True, "low_relevance", self._get_topic_suggestions(search_results)
        
        return False, "", []

    def _get_vague_clarifications(self, query: str) -> List[str]:
        """Generate clarification options for vague queries"""
        clarifications = {
            'workflow': ['creating a new workflow', 'editing an existing workflow', 'running/executing a workflow', 'troubleshooting workflow issues'],
            'activity': ['finding activities to use', 'configuring activity settings', 'activity not working', 'understanding what an activity does'],
            'setup': ['initial Actions setup', 'configuring a specific feature', 'user account setup', 'integrating with other systems'],
            'configure': ['workflow configuration', 'activity configuration', 'system settings', 'user preferences'],
            'action': ['creating actions', 'running actions', 'action configuration', 'action troubleshooting'],
            'help': ['getting started guide', 'specific feature help', 'troubleshooting issues', 'understanding concepts'],
            'issue': ['workflow not working', 'activity errors', 'login problems', 'performance issues'],
            'problem': ['configuration problems', 'execution problems', 'access problems', 'integration problems']
        }
        
        for term, options in clarifications.items():
            if term in query:
                return options
        
        return ['getting started with Actions', 'specific feature configuration', 'troubleshooting an issue', 'understanding concepts']

    def _get_topic_suggestions(self, search_results: List[Any]) -> List[str]:
        """Extract topic suggestions from search results"""
        suggestions = []
        for result in search_results[:4]:
            if hasattr(result, 'page_title'):
                page_title = getattr(result, 'page_title', '')
                if page_title and page_title not in suggestions:
                    suggestions.append(page_title)
        return suggestions

    def _build_disambiguation_response(self, original_query: str, reason: str, options: List[str], context_chunks: List[ContextChunk]) -> str:
        """Build a disambiguation response using existing patterns"""
        
        base_response = "I'd be glad to help you with that! To give you the most relevant information, could you clarify what specifically you're looking for?\n\n"
        
        if reason == "vague_query":
            base_response += "**Are you interested in:**\n"
            for i, option in enumerate(options, 1):
                base_response += f"{i}. {option.title()}\n"
        
        elif reason == "multiple_contexts":
            base_response += "**I found information in several areas:**\n"
            for i, content_type in enumerate(options, 1):
                base_response += f"{i}. {content_type.title()} documentation\n"
        
        elif reason == "low_relevance":
            base_response += "**Here are some related topics that might help:**\n"
            for i, suggestion in enumerate(options[:4], 1):
        ...
        # Add available context if we have some
        if context_chunks:
            base_response += f"\n**Based on what I found, here are some relevant resources:**\n"
            for i, chunk in enumerate(context_chunks[:3], 1):
                base_response += f"- {chunk.page_title}: {chunk.source_url}\n"
    ...
    def _should_use_relationships(self, message: str, question_type: str) -> bool:
        ...
        # Skip relationships for support-routed questions (they don't need documentation search)
        if question_type in ['support_request', 'account_issue', 'system_issue', 'billing_issue', 'billing_followup', 'urgent_issue', 'bug_report']:
            return False
    ...
    def chat(self, chat_request: ChatRequest) -> ChatResponse:
        """Enhanced chat method with comprehensive support routing and disambiguation"""
        start_time = datetime.now()
        
        try:
            # Step 1: Detect question type with enhanced patterns
            ...
            # Step 2: Handle all support-routed questions with appropriate responses
            support_responses = {
                'support_request': SUPPORT_RESPONSE,
                'account_issue': ACCOUNT_ISSUE_RESPONSE,
                'system_issue': SYSTEM_ISSUE_RESPONSE,
                'billing_issue': BILLING_ISSUE_RESPONSE,
                'billing_followup': BILLING_FOLLOWUP_RESPONSE,
                'urgent_issue': URGENT_ISSUE_RESPONSE,
                'bug_report': BUG_REPORT_RESPONSE
            }
            
            if question_type in support_responses:
                ai_response = support_responses[question_type]
                processing_time = (datetime.now() - start_time).total_seconds()
                
                logger.info(f"Routed {question_type} directly to support (no documentation search)")
            ...
            # Step 3: For documentation questions, search with relationship enhancement when beneficial
            ...
            # Step 3.5: Check for disambiguation needs (NEW)
            needs_disambiguation, reason, options = self.needs_disambiguation(chat_request.message, search_response.results)
            
            if needs_disambiguation and options:
                context_chunks = self.build_context_chunks(search_response.results)
                disambiguation_response = self._build_disambiguation_response(chat_request.message, reason, options, context_chunks)
                processing_time = (datetime.now() - start_time).total_seconds()
                
                logger.info(f"Disambiguation triggered: {reason} - offering {len(options)} clarification options")
                
                return ChatResponse(
                    response=disambiguation_response,
                    context_used=context_chunks,
                    sources_count=len(set(chunk.source_url for chunk in context_chunks)),
                    processing_time=processing_time,
                    enhanced_features_used=True,  # Mark as using disambiguation
                    relationship_enhanced_chunks=0
                )
            
            # Step 4: Build context chunks with enhanced metadata and defensive checks
            context_chunks = self.build_context_chunks(search_response.results)
            
            # Step 5: Generate relationship-aware response
            if context_chunks:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "approach": "enhanced_support_routing_with_relationship_search_and_disambiguation",
            ...
            "features": {
                "comprehensive_support_routing": True,
                "account_issue_detection": True,
                "system_issue_detection": True,
                "billing_issue_detection": True,
                "billing_followup_detection": True,
                "urgent_issue_detection": True,
                "bug_report_routing": True,
            ...
            "statistics": {
                ...
                "support_routing_patterns": len([p for patterns in [
                    "support_request", "account_issue", "system_issue", 
                    "billing_issue", "billing_followup", "urgent_issue", "bug_report"
                ] for p in patterns])
            }
        }
    
    def test_connection(self) -> Dict[str, Any]:
        """Test the enhanced Gemini API connection with comprehensive support routing and disambiguation"""
        try:
            if not self.ready:
            ...
            test_response = self.model.generate_content(
                "Respond with: 'I'd be glad to help you test the connection! The enhanced RANI assistant is ready with comprehensive support routing, smart documentation search, and disambiguation capabilities.'"
            )
            ...
            if test_response.text:
                ...
                return {
                    ...
                    "features": "Comprehensive support routing, relationship-enhanced search, documentation-bounded responses, disambiguation, billing follow-up",
                    "relationship_data_status": f"{search_stats.get('chunks_with_relationships', 0)} chunks with relationship data available",
                    "support_routing": "7 categories of issues automatically routed to support",
                    "disambiguation": "3 scenarios automatically detected and handled"
                }
            else:
