...
class GeminiService:
    ...
    def generate_response(self, prompt: str) -> str:
        """Generate response using Gemini with enhanced error handling for complex responses"""
        try:
            if not self.ready:
            ...
            # Enhanced response handling for complex responses
            try:
                ...
                logger.warning(f"Failed to access response.text: {text_access_error}")
                ...
                try:
                    if hasattr(response, 'candidates') and response.candidates:
                        ...
                        if hasattr(candidate, 'content') and candidate.content.parts:
                            ...
                            if text_parts:
                                logger.info("Gemini response extracted from candidates.parts")
            ...
            # If all methods fail
            logger.warning("Gemini returned a response but couldn't extract text")
            return "I'd be glad to help you, though I received an unexpected response format. Could you try rephrasing your question?"
    ...
    def generate_response_with_retry(self, prompt: str, max_retries: int = 3) -> str:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt == max_retries:
                    ...
                    return f"I'd be glad to help you, though I encountered persistent technical issues after {max_retries + 1} attempts. Please try again later or use the support button below."
        ...
        return "I'd be glad to help you, though I encountered unexpected issues. Please try again."
    ...
    def chat(self, chat_request: ChatRequest) -> ChatResponse:
        ...
        try:
            ...
            # Step 4: ALWAYS generate the response using the LLM with retry logic
            ...
            ai_response = self.generate_response_with_retry(prompt)
    ...
    def test_connection(self) -> Dict[str, Any]:
        """Test the enhanced Gemini API connection with robust response handling"""
        try:
            if not self.ready:
            ...
            # Use the same robust response handling for test
            response_text = None
            try:
                ...
                try:
                    ...
                    logger.warning(f"Failed to extract test response: {e}")

            if response_text:
                search_stats = self.search_service.get_search_stats()
                return {
                    "success": True, 
                    "message": "Enhanced Gemini API connection working with robust error handling",
                    "test_response": response_text,
                    "features": "Prompt-based routing, relationship-enhanced search, documentation-bounded responses, account/password disambiguation, enhanced error handling",
                    "relationship_data_status": f"{search_stats.get('chunks_with_relationships', 0)} chunks with relationship data available",
                    "routing_approach": "All routing and disambiguation decisions handled by LLM via sophisticated prompt template with automatic retry logic"
                }
            else:
