...
def get_services() -> Tuple:
    """
    Dependency to get initialized services from main app.
    ...
    if not services_initialized or not all([doc_processor, search_service, gemini_service]):
        raise HTTPException(
            status_code=503, 
            detail="Services not ready. Please wait for application startup to complete."
        )
...
@router.get("/ping")
async def ping():
    """Lightweight ping endpoint to keep service warm"""
    return {
        "status": "pong",
        "timestamp": datetime.now().isoformat()
    }
    ...
    try:
        ...
        if upload_format == "comprehensive":
            ...
            try:
                ...
                logger.info(f"Enhanced features: {comprehensive_request.enhanced_features}")
                ...
                enhanced_result = {
                    **result,
                    ...
                    "documentation_stats": {
                        "generation_timestamp": comprehensive_request.generated,
                        "product": comprehensive_request.product,
                        "version": comprehensive_request.version,
                        "total_pages": comprehensive_request.total_pages,
                        "total_headers": comprehensive_request.total_headers,
                        "enhanced_features_count": len(comprehensive_request.enhanced_features),
                        "variable_substitutions_applied": len(comprehensive_request.variable_substitutions)
                    }
                }
...
async def service_health_check():
    ...
    return {
        "doc_processor_available": main_app.doc_processor is not None,
        "search_service_available": main_app.search_service is not None,
        "gemini_service_available": main_app.gemini_service is not None,
        "services_initialized": main_app.services_initialized,
        "enhanced_features_available": main_app.search_service.get_search_stats().get('enhanced_features', False) if main_app.search_service else False,
        "timestamp": datetime.now().isoformat()
    }
...
async def readiness_check():
    ...
    try:
        # Get services (will fail with 503 if not ready)
