...
const VideoForm = ({ mode = 'create' }) => {
  ...
  const handleChange = e => {
    ...
    // Auto-assign section when category changes
    if (name === 'category') {
      setFormData(prev => ({
        ...prev,
        section: value, // Auto-assign section to match category
      }))
    }
  ...
  const handleCustomTagsChange = value => {
    setCustomTags(value)
    updateFormTags(selectedTags, value)
  }

  const updateFormTags = (selected, custom) => {
    const customArray = custom
      ? custom
          .split(',')
          .map(tag => tag.trim())
          .filter(tag => tag)
      : []
    const allTags = [...selected, ...customArray]
    setFormData(prev => ({
      ...prev,
      tags: allTags.join(', '),
    }))
  ...
  // Remove unused handleLearningPathChange function
  // This function was defined but never used in the component
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      setError(
        'Please fill in all required fields (Custom Video ID, Title, Description, Video URL, Unique URL Code, Category, Platform, Duration, Section Order, Tags)',
      )
      setSaving(false)
      return
    }
        ...
        <section style={styles.section}>
          ...
          <div style={styles.gridTwo}>
            <div style={styles.formGroup}>
              ...
              />
              <small style={styles.helpText}>
                This should match the category where the video appears in the
                documentation
              </small>
            </div>
            ...
            <div style={styles.formGroup}>
              ...
              />
              <small style={styles.helpText}>
                This should match the Category (auto-assigned)
              </small>
            </div>
          </div>
          ...
          <div style={styles.formGroup}>
            ...
            <div style={styles.tagSection}>
              <h4 style={styles.tagSectionTitle}>Frequently Used Tags</h4>
            ...
            {/* Custom Tags */}
            <div style={styles.tagSection}>
              <h4 style={styles.tagSectionTitle}>
              ...
              />
              <small style={styles.helpText}>
                Add any additional tags not available above (comma-separated)
              </small>
            </div>
            ...
            {formData.tags && (
              <div style={styles.tagPreview}>
                <strong>Selected Tags:</strong> {formData.tags}
              </div>
            )}
          </div>
