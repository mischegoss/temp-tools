...
// FIXED: Support and copy buttons function exactly like Pro chatbot
...
const DocusaurusChatbotComponent = ({ isOpen = false, onClose }) => {
  ...
  // FIXED: Get updateConversationState from useChatValidation, not useChatbot
  const { validateAndGetResponse, updateConversationState } =
  ...
  // Validation state tracking - FIXED: Enhanced validation tracking
  const validationState = useRef({
    lastValidatedMessage: '',
    lastValidationType: null,
    consecutiveShortInputs: 0,
    hasShownHelpPrompt: false,
    lastInteractionTime: Date.now(),
  })

  // Update validation state
  const updateValidationState = useCallback((message, validationType) => {
    ...
    validationState.current = {
      ...
      consecutiveShortInputs:
        message.length < 3
          ? validationState.current.consecutiveShortInputs + 1
          : 0,
    }
  }, [])
  ...
  useEffect(() => {
    const handleMouseMove = e => {
      ...
      if (resizeType === 'width') {
        const newWidth = Math.max(350, Math.min(800, e.clientX - rect.left))
    ...
    async (messageText = null) => {
      try {
        ...
        if (!validation.shouldSendToAPI) {
          ...
          updateValidationState(message, validation.type)

          return true
        }

        // For valid messages, send to API
  ...
  // FIXED: Support functionality - matches Pro exactly
  const handleSupportClick = useCallback(() => {
    try {
      // Generate detailed chat summary like Pro
      const chatSummary = `Chat Summary for Support Ticket
Generated: ${new Date().toLocaleString()}
Product: Actions Documentation
...
End of chat summary. Please use this context when responding to the support ticket.`
      ...
      // 2. FIXED: Show notification that chat was copied to share with support
      showToastMessage('Chat copied to share with support', 'success')

      // 3. FIXED: Open support portal like Pro (not mailto)
      setTimeout(() => {
        window.open('https://support.resolve.io/', '_blank')
      }, 1000) // Delay so user sees the notification
  ...
  const styles = {
    ...
    toastStyle: {
      ...
      top: '20px',
      right: '20px',
      backgroundColor: currentToastStyle.backgroundColor,
      color: 'white',
      padding: '16px 24px',
      borderRadius: '8px',
  ...
  const chatWindowProps = {
    ...
    handleFeedback,
    handleClearChat,
    handleCopyChatClick, // FIXED: Now matches Pro exactly
    handleSupportClick, // FIXED: Now matches Pro exactly
    dimensions,
    setDimensions,
    isServerUnavailable,
    connectionError,
