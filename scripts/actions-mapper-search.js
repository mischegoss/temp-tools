...
// Configuration flags - minifier removed
...
Examples:
  node actions-title-scanner.js                    # Generate readable JSON
  node actions-title-scanner.js --verbose          # Show detailed progress
  npm run scan-actions                             # Generate readable JSON
...
`)
  ...
  ;(async () => {
    try {
      ...
      console.log('📝 Generating readable JSON file')

      await scanActionsDocumentation()

      console.log('\n🎉 Actions title scanning completed successfully!')
      console.log('📋 Generated clean, readable JSON file')
    } catch (error) {
