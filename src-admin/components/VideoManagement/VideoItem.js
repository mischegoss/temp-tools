 ...
 * Props:
 * - video: Video object
 * - onEdit: Callback for edit action (receives firestoreId)
 * - onDelete: Callback for delete action (receives video object)
 * - onViewJSON: Callback for view JSON action (receives firestoreId)
 */
const VideoItem = ({ video, onEdit, onDelete, onViewJSON }) => {
  // Use Firestore document ID for database operations
  const firestoreDocId = video.firestoreId
  // Use custom ID for display to users
  const customVideoId = video.customId || video.id // Fallback to video.id for backward compatibility
  ...
  return (
    <div style={styles.container}>
      <div style={styles.content}>
        ...
        {/* Video ID - Display the custom ID to users */}
        <div style={styles.idContainer}>
          <span style={styles.idLabel}>ID:</span>
          <span style={styles.id}>{customVideoId}</span>
        </div>
      ...
      <div style={styles.actions}>
        <button
          ...
          onClick={() => onEdit(firestoreDocId)}
        >
          Edit
        </button>
        <button
          style={styles.jsonButton}
          onClick={() => onViewJSON(firestoreDocId)}
        >
          View JSON
        </button>
