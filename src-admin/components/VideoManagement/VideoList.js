...
const VideoList = () => {
  ...
  const handleSearch = searchTerm => {
    ...
    const filtered = videos.filter(
      video =>
        ...
        video.customId.toLowerCase().includes(term) ||
  ...
  const handleEdit = firestoreId => {
    // Use Firestore document ID for routing to edit page
    navigate(`/videos/edit/${firestoreId}`)
  }

  const handleViewJSON = firestoreId => {
    // Use Firestore document ID for routing to JSON editor
    navigate(`/videos/json/${firestoreId}`)
  }
  ...
  const handleDeleteConfirm = async () => {
    ...
    const result = await deleteVideo(videoToDelete.firestoreId)

    if (result.success) {
      // Remove from local state using Firestore ID
      setVideos(videos.filter(v => v.firestoreId !== videoToDelete.firestoreId))
      setFilteredVideos(
        filteredVideos.filter(v => v.firestoreId !== videoToDelete.firestoreId),
      )
