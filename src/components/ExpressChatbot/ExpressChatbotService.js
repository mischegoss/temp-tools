...
// FIXED: Updated to match actual backend response format
...
class ExpressChatbotService extends BaseChatbotService {
  ...
  async sendMessage(message, conversationHistory = []) {
    ...
    try {
      ...
      return {
        success: true,
        message: result.data.message, // FIXED: Use "message" field
        metadata: {
          responseTime: result.data.processing_time
            ? result.data.processing_time * 1000
            : null, // Convert to ms
          sources: result.data.context_used || [],
          confidence: result.data.confidence || null,
          version: result.data.version_context || currentVersion,
          product: 'express',
          model: result.data.model_used || null,
        },
      }
    } catch (error) {
