...
export function setFilterInURL(filterType) {
  ...
  // Update category link clickability based on filter state
  updateCategoryLinksClickability(filterType)
...
function startUltraRobustPersistence(filterType) {
  ...
  const interval = setInterval(() => {
    updateAllLinksComprehensively(filterType)
    // Also re-apply category link disabling in case DOM changed
    updateCategoryLinksClickability(filterType)
    applySimpleFiltering()
...
export async function applySimpleFiltering() {
  ...
  // Update category link clickability based on filter state
  updateCategoryLinksClickability(activeFilter)
...
/**
 * Disable or enable category links based on filter state
 * When filter is active: ALL category links become unclickable
 * The expand/collapse arrow still works
 */
function updateCategoryLinksClickability(filterType) {
  if (typeof window === 'undefined') return
  ...
  if (filterType === 'none') {
    ...
    console.log('🔓 Category links: clickable (no filter active)')
  } else {
    ...
    console.log('🔒 Category links: disabled (filter active:', filterType, ')')
  }
}
