...
class ChatbotService {
  ...
  /**
   * Send a hidden warmup message to fully exercise the AI pipeline
   * This ensures the AI models are loaded and ready
   */
  async warmupAIModels() {
    try {
      // Send a lightweight test message that exercises the full pipeline
      const testMessage = 'test'
      const result = await this.sendMessage(testMessage, [])

      console.log('🔥 AI models warmed up successfully')
      return result
    } catch (error) {
      console.log('⚠️ AI warmup failed:', error.message)
      throw error
    }
  }
  ...
  async preemptiveWarmup() {
    try {
      ...
      // Also send a test message to warm up AI models
