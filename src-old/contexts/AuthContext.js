...
import { isBrowser, getItem, setItem, removeItem } from '../components/Forms/utils/BrowserStorage';
...
export function AuthProvider({ children, skipAuthCheck = false }) {
  ...
  const authStateCheckCounter = useRef(0);
  ...
  useEffect(() => {
    ...
    if (skipAuthCheck) {
      ...
      // Skip all the auth checks on non-learning paths
      setLoading(false);
      return () => {}; // Empty cleanup function
    }
    
    console.log('[AuthContext] Auth state checking at path:', location.pathname);
    
    // Increment counter to track how many times this effect runs
    authStateCheckCounter.current += 1;
    console.log('[AuthContext] Auth state check counter:', authStateCheckCounter.current);
    
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      console.log('[AuthContext] Auth state changed:', {
        user: currentUser ? `${currentUser.email} (${currentUser.uid})` : 'null',
      ...
      if (currentUser) {
        // Store user data for iframe communication - moved inside useEffect for browser safety
        ...
        if (currentUser.displayName) {
          console.log('[AuthContext] Setting company name from displayName:', currentUser.displayName);
  ...
  }, [user, loading, location, skipAuthCheck]);
...
export function useAuth() {
  try {
    ...
    if (context === undefined) {
      console.warn('[AuthContext] useAuth hook used outside of AuthProvider context');
