...
export default function Root({ children }) {
  ...
  // Dynamically add/remove style to hide navbar when needed
  useEffect(() => {
    if (shouldHideHeader) {
      ...
      if (!document.getElementById('hide-navbar-style')) {
        document.head.appendChild(style);
      }
      
      return () => {
        // Clean up style on unmount or route change
        const existingStyle = document.getElementById('hide-navbar-style');
        if (existingStyle) {
          document.head.removeChild(existingStyle);
        }
      };
    } else {
      // Explicitly ensure the navbar is shown when shouldHideHeader is false
      const existingStyle = document.getElementById('hide-navbar-style');
      if (existingStyle) {
        document.head.removeChild(existingStyle);
      }
      ...
      <AuthProvider skipAuthCheck={!isLearningPath}>
        {!shouldHideHeader && (
          <CookieConsent
            ...
            buttonStyle={{ background: "#16a34a", color: "black", fontSize: "13px", borderRadius: "2px" }}
            declineButtonStyle={{ background: "#86efac", color: "black", fontSize: "13px", borderRadius: "2px" }}
            setDeclineCookie
          >
